...
public interface IFinanceDataService {
     ...
     * @throws FinanceDataException if an error occured creating the account
     */
    IAccount createAccount(IUser user, String fullName,
     ...
     * @param addedDate
     *            the date in which the transaction was created
     * @param type
     ...
     * @throws FinanceDataException if an error occured creating the transaction
     */
    ITransaction createTransaction(IAccount account, long addedDate,
            TransactionType type, String category, BigDecimal amount,
            String reason) throws FinanceDataException;
     ...
     * @throws FinanceDataException if an error occurred retrieving the account
     */
    IAccount getAccount(IUser user, long accountId)
        throws FinanceDataException;

    /**
     * This function finds accounts, populates a list with the accounts, and
     * returns the list.
     * 
     * @param user
     *            the user the list of accounts belong to
     * @return the List populated with requested IAccount objects
     * @throws FinanceDataException if an error occurred retrieving the list of accounts
     */
    List<IAccount> getAccounts(IUser user) throws FinanceDataException;

    /**
     * This function finds transaction, populates a list with the transactions,
     * and returns the list.
     * 
     * @param account
     *            the account the transactions belong to
     * @return the List populated with requested ITransaction objects
     * @throws FinanceDataException if an error occurred retrieving the transactions
     */
    List<ITransaction> getTransactions(IAccount account)
        throws FinanceDataException;

    /**
     * This function finds Category Spending Reports, populations a map with the
     * reports, and returns the map.
     * 
     * @param user
     *            the user that the report generation is for
     * @param startTimestamp
     *            the starting time for the spending report
     * @param endTimestamp
     *            the ending time of the spending report
